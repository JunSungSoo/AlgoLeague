    ...
    async function execute(mode: "run" | "submit") {
        ...
        try {
            ...
            const sampleSummary = Array.isArray(result.sampleTests)
                ? result.sampleTests
                      .map(
                          (sample: { ordinal: number; passed: boolean; durationMs: number }) =>
                              `Sample Test ${sample.ordinal}: ${sample.passed ? "통과" : "실패"} (${sample.durationMs}ms)`,
                      )
                      .join("\n")
                : "";
            setOutput(
                response.ok
                    ? `${result.verdict ?? result.status}\n${result.message}${sampleSummary ? `\n${sampleSummary}` : ""}`
                    : result.error,
            );
